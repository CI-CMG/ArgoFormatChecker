 ...
 */
public abstract class BaseArgoFileValidator implements ArgoFileValidator {
	// class variables
	private static final Logger log = LoggerFactory.getLogger("ArgoFileValidator");

  // ..check for legal data_mode
	private static final Pattern pDataMode = Pattern.compile("[RDA]+"); // ..CDL "variables:" tag

  //Do not make static. DecimalFormat is not thread safe.
	private final DecimalFormat cycleFmt = new DecimalFormat("000");

	protected final Date earliestDate = ArgoDate.get("19970101000000");
	protected final static long oneDaySec = 1L * 24L * 60L * 60L * 1000L;

	// ..object variables
	protected final ArgoDataFile arFile;
	protected ValidationResult validationResult;
  protected final ArgoNVSReferenceTable argoNVSReferenceTable;

	public BaseArgoFileValidator(ArgoDataFile arFile, ArgoNVSReferenceTable argoNVSReferenceTable) {
		this.arFile = arFile;
  ...
  @Override
	...
	private boolean checkSpecialTechParamVarAttributeValue(Variable dataVar, String varName, Attribute dataAttr,
		...
		boolean result = true;
		if (arFile.getFileSpec().getConfigTech() != null && arFile.getFileSpec().getConfigTech().findTechParam(varName) != null) {
...
//				result = checkTechParamLongNameAttribute(dataVar, varName, attrName, dataAttrValue);
	...
	private boolean checkTechParamUnitsAttribute(String varName, String attrName, String dataAttrValue) {
		...
		// CK_0095 2/2
		// ===========
		if (!arFile.getFileSpec().getConfigTech().isConfigTechUnit(dataAttrValue)
				&& !arFile.getFileSpec().getConfigTech().isDeprecatedConfigTechUnit(dataAttrValue)) {
	...
	private boolean checkAttributeType(String varName, Attribute dataAttr, String attrName, ArgoAttribute specAttr) {
		...
		if (specAttr.isString()) {
			...
			throw new IllegalStateException("Unknown specAttr type: " + specAttr);
		}
		return true;
	}
	 ...
	 */
	protected boolean basicDataValidation(boolean ckNulls) throws IOException {
	...
	protected void validatePINAME() {
		...
		if (checkStrVarEmpty("PI_NAME")) {
			...
			for (String piName : piNames) {
				SkosConcept piNameTableEntry = argoNVSReferenceTable.getPI_NAME_TABLE().getConceptMembersByAltLabelMap()
						.get(piName);
	 ...
	 *
	 */
	private String getGdacFileName() {
  ...
  @Override
	 ...
	 * 
	 * @param fillValue for the parameter
	 * @param value       value
	 * @return true = value is FillValue; false = value is not FillValue
	 */
	protected static boolean is_FillValue(float fillValue, float value) {
	 ...
	 * @return true = value is FillValue; false = value is not FillValue
	 */
  protected static boolean is_FillValue(double fillValue, double value) {
	 ...
	 * @return true = value is FillValue; false = value is not FillValue
	 */
  protected static boolean is_99_999_FillValue(float value) {
	 ...
	 * @return true = value is FillValue; false = value is not FillValue
	 */
  protected static boolean is_99_999_FillValue(double value) {
	 ...
	 * @return true = value is FillValue; false = value is not FillValue
	 */
  protected static boolean is_999_999_FillValue(double value) {
